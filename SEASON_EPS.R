...
season_report_eps <- function(file_name_season, y, z) {
  ...
  if(y=='101' | y=='100' | y=='99'){ # year < 102
    f <- html_text(html_nodes(stats, xpath = paste0('//body//center[3]//table//tr[1]//th')))
  }else{
  ...
  for(x in c(2:1200)){
     # x <- 500
    if(y=='101' | y=='100' | y=='99'){ # year < 102
      aaa <- html_text(html_nodes(stats, xpath = paste0('//body//center[3]//table//tr[',x,']//td')))
      if((length(aaa)==0)==FALSE){
        for(xx in c(1:length(aaa))){aaa[xx] <- gsub(',', replacement = '', aaa[xx])}
        f <- rbind(f, aaa )
      }
    }else{
